...
Внутри пакета модуля код раскладывается по подпакетам по назначению класса.
Так должно быть всегда — отступать можно только если пользователь явно попросит
иное.

- `config` — конфигурации (`@Configuration`, `@AutoConfiguration`) и свойства
  (`@ConfigurationProperties`).
- `controllers` — контроллеры (`@Controller`, `@RestController`).
- `services` — сервисы (бизнес-логика, бины-реестры и т. п.).
- `model` — модель (записи/классы данных, DTO).

Точка входа (`Main`) и общие константы модуля остаются в корне пакета.
